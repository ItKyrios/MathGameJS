let currentTopDisplay = document.querySelector('.demoA');
var numpad = {
  ...
  init: () => {
    ...
    numpad.hdisplay.type = "hidden";
    numpad.hdisplay.disabled = false;
    ...
    // OK
    buttonator("<br>&#10149;", "ok", numpad.select);
    // 0
  ...
  digit: (num) => {
    ...
    if (current.length < numpad.nowMax) {
      if (current=="0") { numpad.hdisplay.value = num; currentTopDisplay.value = num;}
      else { numpad.hdisplay.value += num; currentTopDisplay.value += num;}
    }
  },
  ...
  delete: () => {
    ...
    if (length == 1) { 
      numpad.hdisplay.value = 0; 
      currentTopDisplay.value = 0;}
    else { 
      numpad.hdisplay.value = numpad.hdisplay.value.substring(0, length - 1); 
      currentTopDisplay.value = currentTopDisplay.value.substring(0, length - 1);}
  },
